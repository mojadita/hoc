...
INST(constpush,2, SUFF(datum, prog)) /* introduce un valor constante en la pila */
...
INST(eval,2,      SUFF(symb, prog))/* evalua una variable */
INST(assign,2,    SUFF(symb, prog))/* asigna X a una variable */
INST(print,1)                      /* imprime X */
INST(bltin0,2,     SUFF(symb, prog))/* llama a una funcion bltin0 (una sin parametros) */
INST(bltin1,2,     SUFF(symb, prog))/* llama a una funcion bltin1 (de un parametro) */
INST(bltin2,2,     SUFF(symb, prog))/* llama a una funcion bltin2 (de dos parametros) */
...
INST(and_then,2,   SUFF(addr, prog))/* operador Y && X (con cortocircuito) */
INST(or,1)                          /* operador Y || X (sin cortocircuito) */
INST(or_else,2,    SUFF(addr, prog))/* operador Y || X (con cortocircuito) */
INST(readopcode,2, SUFF(symb, prog))/* lee un valor y lo asigna a una variable */
INST(call,3,   SUFF(symb_int, prog))/* llama a una subrutina con los parametros de la pila */
...
INST(argeval,2,    SUFF(arg, prog)) /* evalua un argumento y lo pone en la pila. */
INST(argassign,2,  SUFF(arg, prog)) /* asigna el top de la pila a $n.  X -> $n */
INST(prstr,2,      SUFF(str, prog)) /* imprime una cadena */
...
INST(if_f_goto,2,  SUFF(addr, prog))/* salto si el top de la pila es cero */
INST(Goto,2,       SUFF(addr, prog))/* salto incondicional */
